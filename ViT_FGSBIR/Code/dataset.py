...
def get_dataloader_RGB(hp):

    dataset_Train  = FGSBIR_RGB_Dataset(hp, mode = 'Train')
    dataloader_Train = data.DataLoader(dataset_Train, batch_size=hp.batchsize, shuffle=True,
                                         num_workers=int(hp.nThreads))

    dataset_Test  = FGSBIR_RGB_Dataset(hp, mode = 'Test')
    dataloader_Test = data.DataLoader(dataset_Test, batch_size=1, shuffle=False,
                                         num_workers=int(hp.nThreads))
...
class FGSBIR_RGB_Dataset(data.Dataset):
    def __init__(self, hp, mode):
        ...
        self.train_path = os.path.join(self.root_dir, 'trainA')
        self.test_path = os.path.join(self.root_dir, 'testA')

        self.Train_Sketch = [os.path.splitext(x)[0] for x in os.listdir(self.train_path) if os.path.isfile(os.path.join(self.train_path, x))]
        self.Test_Sketch = [os.path.splitext(x)[0] for x in os.listdir(self.test_path) if os.path.isfile(os.path.join(self.test_path, x))]
    ...
    def __getitem__(self, item):
        sample  = {}
        if self.mode == 'Train':
            sketch_path = os.path.join(self.train_path, self.Train_Sketch[item])
            ...
            sketch_path = os.path.join(self.test_path, self.Test_Sketch[item] + '.png')
            sketch_img = Image.open(sketch_path).convert('RGB')
...
class FGSBIR_RGB_Pic_Dataset(data.Dataset):
    def __init__(self, hp):
        self.hp = hp
        self.root_dir = os.path.join(hp.root_dir, 'Dataset', hp.dataset_name)
        self.test_transform = get_ransform('Test')
        self.pic_path = os.path.join(self.root_dir, 'photo')
        self.Test_Picture = [os.path.splitext(x)[0] for x in os.listdir(self.pic_path) if
                             os.path.isfile(os.path.join(self.pic_path, x))]

    def __getitem__(self, item):
        sample  = {}
        positive_sample = self.Test_Picture[item]
        positive_path = os.path.join(self.root_dir, 'photo', positive_sample + '.png')
        positive_img = self.test_transform(Image.open(positive_path).convert('RGB'))
        sample = {'positive_img': positive_img, 'positive_path': positive_sample}
    ...
    def get_img(self, type, name):
        img_path = os.path.join(self.root_dir, type, name + '.png')
        img = self.test_transform(Image.open(img_path).convert('RGB'))
        return img