...
from .Networks import VGG_Network, InceptionV3_Network, Resnet50_Network, ViT_Network
...
class FGSBIR_Model(nn.Module):
    ...
    # def get_top(self, dataloader_pic, dataset_Sketch, item):
    #     Image_Feature_ALL = []
    #     Image_Name = []
    #     for i_batch, sampled_batch in enumerate(dataloader_pic):
    ...
    #                 Image_Feature_ALL.append(positive_feature[i_num])
    #     Image_Feature_ALL = torch.stack(Image_Feature_ALL)
    #     sketch_img = dataset_Sketch[item]['sketch_img']
    #     sketch_feature = self.sample_embedding_network(sketch_img.to(device)).cpu()
    #     distance = F.pairwise_distance(sketch_feature.unsqueeze(0), Image_Feature_ALL)
    #     distance_list = distance.squeeze(0).tolist()
    #     distance_map = dict(zip(Image_Name, distance_list))
    #     ranking = sorted(distance_map.items(), key= lambda x:x[1], reverse=True)
    #     return ranking[0], ranking[:10]

    def get_top(self, dataset_pic, sketch_img):
        Image_Feature_ALL = []
        Image_Name = []
        for sampled_batch in dataset_pic:
            positive_feature= self.sample_embedding_network(sampled_batch['positive_img'].to(device)).cpu()
            if sampled_batch['positive_path'] not in Image_Name:
                Image_Name.append(sampled_batch['positive_path'])
        ...
        distance_list = distance.squeeze(0).tolist()
        distance_map = dict(zip(Image_Name, distance_list))
        ranking = sorted(distance_map.items(), key= lambda x:x[1], reverse=False)
        return ranking[:10]
